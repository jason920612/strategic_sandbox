...
3. Read the milestone notes here **in order** (M0.2 → M0.10 → M1.1
   ...
   → M4.11 → M4.12 → M4.13 → M4.14 → M4.15 → M4.16). They build on each other
...
Per the M-pacing rule the next sub-milestone (M4.17) waits for
an explicit reviewer green-light. Candidates: full ARIA polish
(role / aria-label / aria-selected) on the clickable +
focusable markers, hover state / tooltips reusing the same

...
3. Read the milestone notes here **in order** (M0.2 → M0.10 → M1.1
   ...
   snapshot at the M4.9 moment) → M4.10 → M4.11 → M4.12 → M4.13). They build on each other
...
Per the M-pacing rule the next sub-milestone (M4.14) waits for
an explicit reviewer green-light. Candidates: hover state /
tooltips reusing the same XSS-safe DOM API, keyboard navigation
/ focus ring / `aria-*` polish, selection persistence on the

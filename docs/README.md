...
3. Read the milestone notes here **in order** (M0.2 → M0.10 → M1.1
   ...
   → M4.11 → M4.12 → M4.13 → M4.14 → M4.15 → M4.16 → M4.17 → M4.18 (+ refreshed `milestone-4-checkpoint.md` snapshot) → M4.19). They build on each other
...
Per the M-pacing rule the next sub-milestone (M4.20) waits for
...
announcements), pair-hover (JS `mouseover`/`mouseout` listener
to highlight matching circle+text when either is hovered),
tooltips reusing the same XSS-safe DOM API, selection
persistence on the

...
3. Read the milestone notes here **in order** (M0.2 → M0.10 → M1.1
   ...
   → M4.11 → M4.12 → M4.13 → M4.14 → M4.15 → M4.16 → M4.17 → M4.18 (+ refreshed `milestone-4-checkpoint.md` snapshot) → M4.19 → M4.20 → M4.21). They build on each other
...
Per the M-pacing rule the next sub-milestone (M4.22) waits for
an explicit reviewer green-light. Candidates: broader ARIA
polish (`aria-selected` on the M4.12 `.selected` markers,
`aria-live` region on the details panel or hover-status bar
for update announcements), pair-hover (JS classList toggling
on the sibling sharing data-id when either is hovered),
position-aware floating tooltip near cursor, mobile-only
responsive layouts beyond the M4.21 SVG-scale rule, selection
persistence on the clicked node, neighbour adjacency edges,
darker dark-mode variant via `prefers-color-scheme`.

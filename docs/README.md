...
3. Read the milestone notes here **in order** (M0.2 → M0.10 → M1.1
   ...
   → M4.11 → M4.12 → M4.13 → M4.14 → M4.15 → M4.16 → M4.17 → M4.18 (+ refreshed `milestone-4-checkpoint.md` snapshot) → M4.19 → M4.20 → M4.21 → M4.22 (+ close-out readiness assessment in `milestone-4-checkpoint.md`)). They build on each other
...
Per the M-pacing rule the next sub-milestone after M4.22 is one
of three reviewer-determined paths: (1) **M4 close-out** PR
publishing `docs/milestone-4-result.md` (mirror
`milestone-3-result.md`'s shape) + flipping the three READMEs
to "M4 closed"; (2) **one more polish PR** from checkpoint
section 9.2 category B (broader ARIA polish — `aria-selected` on
the M4.12 `.selected` markers, `aria-live` region on the
details panel or hover-status bar; pair-hover via JS classList
toggling on the sibling sharing data-id; position-aware
floating tooltip near cursor; mobile-only responsive layouts
beyond the M4.21 SVG-scale rule; selection persistence on the
clicked node; keyboard polish — arrow-key nav, Escape-to-clear;
dark-mode variant via `prefers-color-scheme`); or (3) **stop M4
and move to M5** (neighbour adjacency edges, terrain
overlays, ownership dynamics — Category A items that belong in
M5+ gameplay scope). M4.22 does NOT pick.

...
TEST_CASE("serialize: empty GameState produces a well-formed JSON object") {
    ...
    CHECK(text.front() == '{');
    CHECK(text.find("\"save_version\": 19")         != std::string::npos);
...
TEST_CASE("deserialize: rejects an unknown save_version (v99)") {
    ...
    CHECK(r.error().find("supports 19") != std::string::npos);
...
TEST_CASE("deserialize: an old v11 save is rejected loudly (M4.1 bumped to v12)") {
    ...
    // the rejection message now says "supports 19" instead of
...
TEST_CASE("deserialize: country id at exactly INT_MAX is accepted") {
    ...
    const std::string text = R"({
        ...
        "provinces": [],
        "events": [],
        "event_history": [], "relationships": [], "pending_player_events": [],
        "faction_demands": []
    })";
    const auto r = ss::deserialize(text);
    REQUIRE(r.ok());
...
TEST_CASE("M5.1 deserialize: trigger target not in allowlist rejected") {
    const std::string text = R"({
        ...
        "provinces": [],
        "events": [
          { "id_code": "x", "name": "X", "description": "", "visible_report": "test report", "true_cause": "test cause", "category": "test",
            "triggers": [
...
TEST_CASE("M5.1 deserialize: duplicate event id_code rejected") {
    const std::string text = R"({
        ...
        "provinces": [],
        "events": [
          { "id_code": "dup", "name": "First", "description": "", "visible_report": "dup report 1", "true_cause": "dup test 1", "category": "test",
            "triggers": [
              { "target": "country.stability", "op": "lt", "value": 0.5 }
            ], "effects": [], "weight_modifiers": [], "options": [], "followup_event_ids": [] },
          { "id_code": "dup", "name": "Second", "description": "", "visible_report": "dup report 2", "true_cause": "dup test 2", "category": "test",
            "triggers": [
...
TEST_CASE("M5.4 deserialize: M5.4 does NOT cross-check event_id_code against state.events") {
    ...
    const std::string text = R"({
        ...
        "relationships": [], "pending_player_events": [],
        "faction_demands": []
    })";
    const auto r = ss::deserialize(text);
    REQUIRE(r);
...
TEST_CASE("M6.1 serialize: every events[] entry serializes its true_cause field") {
    GameState state;
    {
        ...
        ev.visible_report = "Reports indicate growing unrest.";
        ev.true_cause     = "The country's actual stability has fallen below the unrest threshold.";
        ev.category       = "domestic_unrest";   // issue #112
        leviathan::core::EventTrigger t;
        t.target = "country.stability";
        t.op     = "lt";
        t.value  = 0.30;
...
TEST_CASE("M6.1 deserialize: a v14 save (no true_cause) is rejected loudly (supports 19)") {
    const std::string text = R"({
        "save_version": 14,
        "rng_algorithm_version": 1,
        "current_date": "1930-01-01",
...
TEST_CASE("M6.2 deserialize: a v15 save (no visible_report) is rejected loudly (supports 19)") {
    const std::string text = R"({
        "save_version": 15,
        "rng_algorithm_version": 1,
        "current_date": "1930-01-01",

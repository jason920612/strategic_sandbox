...
TEST_CASE("serialize: empty GameState produces a well-formed JSON object") {
    ...
    CHECK(text.front() == '{');
    CHECK(text.find("\"save_version\": 17")         != std::string::npos);
...
TEST_CASE("deserialize: rejects an unknown save_version (v99)") {
    ...
    CHECK(r.error().find("supports 17") != std::string::npos);
...
TEST_CASE("deserialize: an old v11 save is rejected loudly (M4.1 bumped to v12)") {
    ...
    // the rejection message now says "supports 17" instead of
...
TEST_CASE("M6.1 deserialize: a v14 save (no true_cause) is rejected loudly (supports 17)") {
    const std::string text = R"({
        "save_version": 14,
        "rng_algorithm_version": 1,
        "current_date": "1930-01-01",
...
TEST_CASE("M6.2 deserialize: a v15 save (no visible_report) is rejected loudly (supports 17)") {
    const std::string text = R"({
        "save_version": 15,
        "rng_algorithm_version": 1,
        "current_date": "1930-01-01",
